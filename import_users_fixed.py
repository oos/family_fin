#!/usr/bin/env python3
"""
Import users and permissions with proper conflict handling
"""
    ...
    try:
        ...
        for user in users:
            try:
                ...
                prod_cursor.execute(f'INSERT INTO "user" ({columns_str}) VALUES ({placeholders}) ON CONFLICT (id) DO UPDATE SET username = EXCLUDED.username, email = EXCLUDED.email, password_hash = EXCLUDED.password_hash, is_active = EXCLUDED.is_active, created_at = EXCLUDED.created_at, updated_at = EXCLUDED.updated_at, role = EXCLUDED.role, password = EXCLUDED.password', values)
                success_count += 1
...
def main():
    print("=== USER AND PERMISSION IMPORT SCRIPT (FIXED) ===")
