#!/usr/bin/env python3
"""
Corrected final script to import GL data with proper totals
"""
...
def import_gl_corrected_final():
    """Import GL data with correct totals (no double counting)"""
    with app.app_context():
        try:
            print("Starting corrected final GL data import...")
            ...
            for gl_account, transactions in gl_accounts.items():
                ...
                # Create Change transaction with CORRECT totals (not double counting)
                change_transaction = TaxReturnTransaction(
                    ...
                    source='GL',
                    annotation='',
                    debit=total_debit,  # Use calculated totals, not doubled
                    credit=total_credit,  # Use calculated totals, not doubled
                    balance=net_change,  # Use calculated net change
                    category_heading=None
                )
            ...
            print("2024 GL data imported with correct totals!")
            ...
            print(f"Error in corrected final import: {e}")
            db.session.rollback()
            import traceback
            traceback.print_exc()

if __name__ == '__main__':
    import_gl_corrected_final()
