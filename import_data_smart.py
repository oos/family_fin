#!/usr/bin/env python3
"""
Smart data import script that handles existing data and foreign key constraints
"""
...
def get_table_dependencies():
    """Define table import order based on foreign key dependencies"""
    return [
...
def import_table_data(local_cursor, prod_cursor, table_name):
    """Import data from local SQLite to production PostgreSQL"""
    try:
        ...
        # Clear existing data (except for user table to preserve admin)
        if table_name != 'user':
...
def main():
    print("=== SMART DATA IMPORT SCRIPT ===")
    ...
    try:
        # Get import order
        import_order = get_table_dependencies()
        print(f"Import order: {import_order}")
        
        # Import tables in dependency order
        success_count = 0
        total_count = len(import_order)
        
        for table_name in import_order:
            if import_table_data(local_cursor, prod_cursor, table_name):
                success_count += 1
            else:
