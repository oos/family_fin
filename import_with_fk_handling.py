#!/usr/bin/env python3
"""
Import data with proper foreign key constraint handling
"""
...
def clear_tables_in_order(prod_cursor):
    ...
    for table in clear_order:
        try:
            prod_cursor.execute(f'DELETE FROM "{table}"')
            print(f"  Cleared {table}")
        except Exception as e:
            print(f"  Error clearing {table}: {e}")

def import_table_data(local_cursor, prod_cursor, table_name):
    """Import table data from local to production"""
    try:
...
def main():
    print("=== DATA IMPORT WITH FK HANDLING ===")
    ...
    try:
        # Clear tables in reverse dependency order
        clear_tables_in_order(prod_cursor)
